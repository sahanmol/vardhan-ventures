...
app.use('/api/contact', rateLimit({ windowMs: 15 * 60 * 1000, limit: 20, standardHeaders: true, legacyHeaders: false, message: { message: 'Too many enquiries. Please try again later.' } }))
...
app.post('/api/contact', [
  ...
  body('service').trim().isLength({ min: 2, max: 80 }).withMessage('Service is required'),
  body('message').trim().isLength({ min: 10, max: 3000 }).withMessage('Message must be between 10 and 3000 characters'),
], async (req, res) => {
  const errors = validationResult(req)
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() })
  try {
    const lead = process.env.MONGODB_URI ? await Contact.create(req.body) : null
...
app.listen(port, () => console.log(`Vardhan Ventures API listening on ${port}`))
